...
`NightlifeItem` is preserved in `types/index.ts` for backward compatibility with existing UI components. New features should consume projections from `domains/event/projections.ts`. A future migration step will remove `NightlifeItem` once all components are projection-driven.
