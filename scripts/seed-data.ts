...
const BASE_URL = getServerEnv().API_URL ?? "http://localhost:5000";
...
async function seedTemporalQueries(): Promise<void> {
  ...
  for (const preset of TEMPORAL_PRESETS) {
    ...
    if (res.ok) {
      ...
      console.warn(
        `  Failed preset ${preset}: ${res.status} ${res.statusText}`,
      );
    }
  }

  console.log(
    `  Temporal queries seeded: ${seeded}/${TEMPORAL_PRESETS.length}`,
  );
}
