    ...
    switch (category) {
      case 'nightlife': // Angular, sharp
        return (
          ...
          >
            <div className={`w-full h-full border-2 ${colorClass.replace('text-', 'border-')}/40 rounded-sm rotate-45`} />
          </motion.div>
        );
      case 'tech': // Bloom, radiant
        return (
          <div className="absolute inset-0 flex items-center justify-center">
            <div className={`w-1 h-full ${colorClass.replace('text-', 'bg-')}/40 animate-pulse`} />
            <div className={`w-full h-1 ${colorClass.replace('text-', 'bg-')}/40 animate-pulse`} />
          </div>
        );
      case 'culture': // Radiant, layered
        return (
          <div className="absolute inset-0 flex items-center justify-center">
            <div className={`w-full h-full border-2 ${colorClass.replace('text-', 'border-')}/40 rounded-full scale-75`} />
            <div className={`w-full h-full border ${colorClass.replace('text-', 'border-')}/20 rounded-full scale-110`} />
          </div>
        );
      ...
      default:
        return <div className={`w-2 h-2 ${colorClass.replace('text-', 'bg-')}/40 rounded-full`} />;
    }
  };
        ...
        >
          ...
          `}>
            ...
            <div className="relative w-4 h-4 flex items-center justify-center">
              {getSignalGlyph()}
              <div className={`w-1.5 h-1.5 rounded-full ${colorClass.replace('text-', 'bg-')} shadow-[0_0_10px_rgba(var(--brand-primary-rgb),0.8)] ${isSelected ? 'scale-150' : ''} transition-transform duration-300`} />
            </div>
            
            {/* Soft Pulse */}
            <motion.div 
              animate={{ scale: [1, pulseIntensity, 1], opacity: [0.3, 0.7, 0.3] }}
              transition={{ duration: 2, repeat: Infinity }}
...
const GhostMarker = ({ event, onDrag }: { event: any, onDrag: (lat: number, lng: number) => void }) => {
...
export default function RadarMap({ 
  ...
  useEffect(() => {
    if (selectedEventId && mapRef.current) {
      ...
      if (event) {
        ...
        const point = map.project([parseFloat(event.longitude), parseFloat(event.latitude)]);
        
        // Only update if the point has actually moved significantly
        const hasMoved = !lastAnchorPoint.current || 
  ...
  const floatingFlyers = useMemo(() => {
    ...
    const inBounds = events.filter(e => {
      ...
      return lat >= mapBounds[1] && lat <= mapBounds[3] && lng >= mapBounds[0] && lng <= mapBounds[2] && e.id !== selectedEventId;
    });
    // Pick up to 3
  ...
  const handleMove = useCallback((evt: any) => {
    ...
    if (mapRef.current) {
      ...
      if (bounds) {
        setMapBounds(bounds.toArray().flat() as [number, number, number, number]);
        if (onBoundsChange) {
          onBoundsChange({
      ...
      if (layers) {
        layers.forEach(layer => {
          ...
          if (['line', 'fill', 'fill-extrusion'].includes(layer.type)) {
            try {
              map.setPaintProperty(layer.id, `${layer.type}-opacity` as any, 0.45); // Increased opacity for readability
            } catch (e) {
  ...
  return (
    <div ref={containerRef} className="absolute inset-0 bg-[#020202] overflow-hidden z-[1]">
      {/* TONIGHT / NOW CHIP */}
      <div className="absolute top-6 left-1/2 -translate-x-1/2 z-50 pointer-events-none">
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-black uppercase italic tracking-tighter text-white">Live Radar</span>
          </div>
          <div className="w-px h-3 bg-white/10" />
          <span className="text-[10px] font-mono text-white/60 uppercase tracking-widest">
            {isClient && new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: false })}
          </span>
        </motion.div>
      </div>
              ...
              return (
                <MarkerErrorBoundary key={cluster.properties.eventId}>
                  <LightLanguageMarker 
                    ...
                    isInterested={interestedEventId === cluster.properties.eventId}
      ...
      {mapError && (
        <div className="absolute inset-0 z-[150] flex items-center justify-center bg-black/80 backdrop-blur-md">
          <div className="p-8 border border-red-500/50 bg-black rounded-3xl text-center max-w-lg">
            <h2 className="text-red-500 text-2xl font-black mb-4 uppercase italic tracking-tighter">Map Render Failed</h2>
