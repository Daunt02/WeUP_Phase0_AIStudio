...
import { NightlifeItem, SocialInviteTier } from '@/types';
import { X, Zap, Users, Lock, EyeOff, MapPin, Calendar, ArrowRight, ShieldCheck } from 'lucide-react';
...
export default function SocialSignalPanel({ isOpen, event, onClose, unlockedTiers }: SocialSignalPanelProps) {
        ...
        >
          ...
          <div className="p-8 pb-4 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-xl font-black uppercase italic text-white leading-none">Interest Saved</h3>
                <p className="text-[9px] font-mono text-brand-primary/60 tracking-[0.3em] uppercase font-bold">Social Signal Active</p>
              </div>
            </div>
          ...
          <div className="px-8 py-4">
            <div className="relative aspect-[16/9] rounded-2xl overflow-hidden border border-white/10 group">
              ...
              <div className="absolute bottom-4 left-4 right-4">
                <h4 className="text-lg font-black uppercase italic text-white leading-tight truncate">{event.title}</h4>
          ...
          <div className="flex-1 px-8 py-4 overflow-y-auto no-scrollbar space-y-6">
            ...
            <div className="space-y-4">
              {MOCK_TIERS.map((tier) => {
                ...
                if (isHidden) {
                  return (
                    <div key={tier.id} className="p-6 rounded-3xl bg-white/[0.02] border border-dashed border-white/5 flex items-center justify-between opacity-40">
                      <div className="flex items-center gap-4">
                        <div className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center">
                          <EyeOff className="w-5 h-5 text-white/20" />
                        </div>
                        <div>
                          <p className="text-[10px] font-black uppercase italic text-white/40 tracking-tighter">Tier {tier.tier_level} Invites</p>
                          <p className="text-[8px] font-mono text-white/20 uppercase tracking-widest">Hidden Layer</p>
                        </div>
                      </div>
                    </div>
                  );
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-3">
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center ${isUnlocked ? 'bg-brand-primary/20 text-brand-primary' : 'bg-white/10 text-white/40'}`}>
                          {isUnlocked ? <ShieldCheck className="w-5 h-5" /> : <Lock className="w-5 h-5" />}
                        </div>
                        <div>
                          <p className={`text-[10px] font-black uppercase italic tracking-tighter ${isUnlocked ? 'text-brand-primary' : 'text-white/40'}`}>
                            Tier {tier.tier_level}: {tier.invite_type}
                          </p>
                          <h5 className="text-sm font-black uppercase italic text-white leading-none mt-1">{tier.title}</h5>
                        </div>
                      </div>
                    </div>
