...
describe("Flow 1: Spatial Discovery", () => {
  ...
  it("MapFeedRequest supports canonical locality filters", () => {
    const request: MapFeedRequest = {
      bounds: { minLat: 29.69, maxLat: 29.8, minLng: -95.45, maxLng: -95.3 },
      locality: {
        marketCode: "houston",
    ...
    expect(request.locality?.marketCode).toBe("houston");
    expect(request.locality?.districtCode).toBe("downtown");
    expect(request.locality?.neighborhoodCode).toBe("market-square");
  });
...
describe("Flow 3: Temporal Filtering", () => {
  const VALID_PRESETS = ["Today", "Tonight", "Weekend", "Next7Days"];

  it.each(VALID_PRESETS)(
    'TemporalPresetRequest accepts preset "%s"',
    (preset) => {
...
describe("Flow 4: Analytics Recording", () => {
  ...
  it.each(ANALYTICS_EVENT_TYPES)(
    'RecordAnalyticsRequest accepts eventType "%s"',
    (eventType) => {
...
describe("Flow 6: Complete User Journey", () => {
  it("journey contracts are all composable", () => {
    ...
    expect(temporalRequest.preset).toBe("Today");
    expect(temporalResponse.timezone).toBe("America/Chicago");
