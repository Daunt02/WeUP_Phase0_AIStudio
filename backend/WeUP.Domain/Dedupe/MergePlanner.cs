...
public sealed class MergePlanner : IMergePlanner
{
        ...
        EventAggregate canonical)
    {
        ...
        var titleScore = GetContractScore(assessment.Scores, ContractDedupe.MatchDimension.Title);
        var timeScore = GetContractScore(assessment.Scores, ContractDedupe.MatchDimension.StartTime);
        var locationScore = GetContractScore(assessment.Scores, ContractDedupe.MatchDimension.Location);

        var conflicts = new List<ContractDedupe.ConflictDescriptor>();

        var titleAction = ResolveContractAction(
            titleScore,
            candidate.Title,
            canonical.Title,
            "Title",
        ...
        var locationAction = ResolveContractAction(
            locationScore,
        ...
        var timeAction = ResolveContractAction(
            timeScore,
        ...
        var autoApply = assessment.Level == ContractDedupe.DuplicateAssessmentLevel.ExactDuplicate
            || IsAboveContractThreshold(assessment);

        return Task.FromResult(new ContractDedupe.MergePlan(
            CandidateId: candidate.CandidateId,
            CanonicalEventId: Guid.TryParse(canonical.CanonicalEventId, out var canonicalEventId)
                ? canonicalEventId
                : Guid.Empty,
    ...
    private static float GetContractScore(ContractDedupe.MatchScoreBreakdown[] scores, ContractDedupe.MatchDimension dimension)
        => scores.FirstOrDefault(s => s.Dimension == dimension)?.Score ?? 0f;

    private static bool IsAboveContractThreshold(ContractDedupe.DuplicateAssessment assessment)
        => assessment.Scores.Length > 0 && assessment.Scores.Average(s => s.Score) >= 0.93f;

    private static ContractDedupe.MergeFieldAction ResolveContractAction(
        float similarity,
